    ...
    /** 
	  ...
	  */
      static boolean isClone(String location0, String location1) {
        ...
        int colDiff = Math.abs(location0.charAt(0) - location1.charAt(0));
        int rowDiff = Math.abs(location0.charAt(1) - location1.charAt(1));

        // if the columnDif and the rowDif is less than 1，then it is a clone move （(0,1}]）
        return (colDiff <= 1 && rowDiff <= 1) && ((colDiff > 0 && rowDiff >= 0) || (colDiff >= 0 && rowDiff > 0));
...
//        return false;
    ...
    static boolean isJump(String location0, String location1) {
        ...
        if (Math.abs(row0 - row1) > 2 || Math.abs(col0 - col1) > 2 ) {
            return false;
        }
        return !isClone(location0, location1);
    }
