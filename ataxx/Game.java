...
class Game {
    ...
    int play() {
        ...
        while (exit < 0) {
            if (ataxxBoard.getWinner() == null) {
                winnerAnnounced = false;
                try {
                    PieceState state = ataxxBoard.nextMove();
                    Player p = getAtaxxPlayer(state);
                    String cmndStr = p.getAtaxxMove();
                    runCommand(cmndStr);
     ...
     * 开始一局新的游戏
     *
     * @param redIndex  红色方是 人 或者 ai
     * @param blueIndex 红色方是 人 或者 ai
     */
    void newGame(int redIndex, int blueIndex) {
        if (redIndex == 0 && blueIndex == 0) { //manual vs manual
            setManual(RED);
            setManual(BLUE);
        }
        if (redIndex == 0 && blueIndex == 1) { //manual vs manual
            setManual(RED);
            setAI(BLUE);
        }
        if (redIndex == 1 && blueIndex == 0) { //manual vs manual
            setAI(RED);
            setManual(BLUE);
        }
        if (redIndex == 1 && blueIndex == 1) { //manual vs manual
            setAI(RED);
            setAI(BLUE);
        }
        exit = -1;
        winnerAnnounced = false;
        view.update(ataxxBoard);
    }

    /**
     * 手动执行一步棋子
     *
     * @param promptData a1-b1 这样的数据
     */
    void manualRun(String promptData) {
        runCommand(promptData);
        if (ataxxBoard.getWinner() == null) {
            winnerAnnounced = false;
        } else if (!gameNotOver()) {
            if (!winnerAnnounced) {
